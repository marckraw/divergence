...
export function buildTerminalSession(
  input: BuildTerminalSessionInput
): TerminalSession {
  const {
    type,
    target,
  ...
  const projectId =
    type === "project" ? target.id : (target as Divergence).projectId;
  const projectSettings = settingsByProjectId.get(projectId);

  const useTmux = projectSettings?.useTmux ?? DEFAULT_USE_TMUX;
  const tmuxHistoryLimit =
    projectSettings?.tmuxHistoryLimit ?? globalTmuxHistoryLimit;
  ...
  const branchName =
    type === "divergence" ? (target as Divergence).branch : undefined;
  ...
  const portEnv = portAllocation
    ? buildPortEnvVars(
        portAllocation.port,
        portAllocation.framework,
        portAllocation.proxyHostname
      )
    : undefined;
...
export function buildWorkspaceTerminalSession(
  input: BuildWorkspaceSessionInput
): TerminalSession {
