...
interface UseWorkspaceSessionRegistryParams {
  ...
  setSplitBySessionId: React.Dispatch<
    React.SetStateAction<Map<string, SplitSessionState>>
  >;
  notificationCallbacksRef: React.MutableRefObject<SessionNotificationCallbacks>;
}

interface UseWorkspaceSessionRegistryResult {
  ...
  statusBySessionRef: React.MutableRefObject<
    Map<string, TerminalSession["status"]>
  >;
  commandBySessionIdRef: React.MutableRefObject<
    Map<string, (command: string) => void>
  >;
...
export function useWorkspaceSessionRegistry({
  settingsByProjectId,
  projectsById,
  appSettings,
...
}: UseWorkspaceSessionRegistryParams): UseWorkspaceSessionRegistryResult {
  const [sessions, setSessions] = useState<Map<string, TerminalSession>>(
    new Map()
  );
  const [activeSessionId, setActiveSessionId] = useState<string | null>(null);
  const [reconnectBySessionId, setReconnectBySessionId] = useState<
    Map<string, number>
  >(new Map());

  const sessionsRef = useRef<Map<string, TerminalSession>>(sessions);
  const activeSessionIdRef = useRef<string | null>(activeSessionId);
  const statusBySessionRef = useRef<Map<string, TerminalSession["status"]>>(
    new Map()
  );
  const commandBySessionIdRef = useRef<Map<string, (command: string) => void>>(
    new Map()
  );
    ...
    setSessions((prev) => {
      ...
      for (const [id, session] of next) {
        ...
        const effectiveHistoryLimit =
          projectHistoryLimit ?? appSettings.tmuxHistoryLimit;
  ...
  const createSession = useCallback(
    (type: "project" | "divergence", target: Project | Divergence) => {
      ...
      const portAllocation =
        portAllocationByEntityKey.get(`${type}:${target.id}`) ?? null;
  ...
  const createManualSession = useCallback(
    (type: "project" | "divergence", target: Project | Divergence) => {
      ...
      const workspaceSessions = Array.from(sessionsRef.current.values()).filter(
        (session) => session.type === type && session.targetId === target.id
      );
      const manualIndex =
        workspaceSessions.filter((session) => session.sessionRole === "manual")
          .length + 1;
  ...
  const handleCreateAdditionalSession = useCallback(
    (type: "project" | "divergence", item: Project | Divergence) => {
  ...
  const handleCloseSessionAndKillTmux = useCallback(
  ...
  const handleRegisterTerminalCommand = useCallback(
    (sessionId: string, sendCommand: (command: string) => void) => {
  ...
  const waitForSessionCommand = useCallback(
    (sessionId: string, timeoutMs = 8000): Promise<(command: string) => void> => {
      ...
      return new Promise((resolve, reject) => {
        const poll = () => {
          ...
          if (Date.now() - startedAt >= timeoutMs) {
            reject(
              new Error(
                "Terminal is not ready - the session may still be loading or its shell failed to start. Try clicking the session tab or reconnecting."
              )
            );
            return;
          }
  ...
  const sendCommandToSession = useCallback(
    async (
      sessionId: string,
      command: string,
      options?: {
      ...
      const shouldActivate =
        activateIfNeeded && previousActiveSessionId !== sessionId;
  ...
  const closeSessionsForProject = useCallback(
    (projectId: number) => {
  ...
  const closeSessionsForDivergence = useCallback(
    (divergenceId: number) => {
      const sessionsToClose = Array.from(sessionsRef.current.entries())
        .filter(
          ([, session]) =>
            session.type === "divergence" && session.targetId === divergenceId
        )
        .map(([sessionId]) => sessionId);
      sessionsToClose.forEach(handleCloseSession);
    },
  ...
  const closeSessionsForWorkspaceDivergence = useCallback(
    (wdId: number) => {
      const sessionsToClose = Array.from(sessionsRef.current.entries())
        .filter(
          ([, session]) =>
            session.type === "workspace_divergence" && session.targetId === wdId
        )
        .map(([sessionId]) => sessionId);
      sessionsToClose.forEach(handleCloseSession);
    },
