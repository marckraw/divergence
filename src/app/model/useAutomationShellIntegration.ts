...
interface UseAutomationShellIntegrationParams {
...
interface UseAutomationShellIntegrationResult {
...
export function useAutomationShellIntegration({
  automations,
  automationRuns,
  projects,
  appSettings,
  ...
  saveAutomation,
  removeAutomation,
}: UseAutomationShellIntegrationParams): UseAutomationShellIntegrationResult {
  void automationRuns;
  const [queuedCloudCountByAutomationId, setQueuedCloudCountByAutomationId] =
  ...
  const executeAutomationRun = useCallback(
    async (
      automationId: number,
      input?: {
      ...
      if (
        ...
        automation.sourceProjectId &&
        automation.targetProjectId
      ) {
        ...
        if (!sourceProject || !targetProject) {
          throw new Error(
            "Source or target project for event automation was not found."
          );
        }
      ...
      try {
        ...
        const eventAutomations = automations.filter(
          (automation) =>
            automation.enabled &&
            automation.runMode === "event" &&
            Boolean(automation.sourceProjectId)
        );
        ...
        await Promise.all(
          sourceProjectIds.map(async (projectId) => {
        ...
        for (const automation of eventAutomations) {
          ...
          const baseBranches = parseGithubTriggerBaseBranches(
  ...
  const handleCloudAutomationEvents = useCallback(
    async (events: GithubPrMergedAutomationEvent[]): Promise<void> => {
      ...
      for (const event of events) {
        try {
          ...
          try {
            ...
            console.warn(
              `Failed to nack cloud automation event ${event.eventId}:`,
              nackError
            );
          }
        }
  ...
  const handleCloudAutomationPollError = useCallback(
    (error: unknown): void => {
  ...
  const handleCloudAutomationEventsPulled = useCallback(
    (events: GithubPrMergedAutomationEvent[]): void => {
  ...
  useCloudAutomationEventPoller({
    enabled: Boolean(
      (appSettings.cloudApiToken ?? "").trim() &&
        (appSettings.cloudApiBaseUrl ?? "").trim()
    ),
    cloudApiBaseUrl: appSettings.cloudApiBaseUrl ?? "",
    cloudApiToken: appSettings.cloudApiToken ?? "",
