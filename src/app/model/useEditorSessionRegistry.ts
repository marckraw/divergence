...
import { buildEditorSession, findEditorSessionByFilePath } from "../../entities";
...
interface UseEditorSessionRegistryResult {
  ...
  applyEditorSessionViewState: (
    sessionId: string,
    viewState: EditorSessionViewState
  ) => Promise<void>;
...
function buildDefaultRuntimeState(
  activeTab: EditorSessionRuntimeState["activeTab"] = "edit"
...
export function useEditorSessionRegistry(): UseEditorSessionRegistryResult {
  const [editorSessions, setEditorSessions] = useState<Map<string, EditorSession>>(
    new Map()
  );
  const [editorViewStateBySessionId, setEditorViewStateBySessionId] = useState<
    Map<string, EditorSessionViewState>
  >(new Map());
  const [editorRuntimeStateBySessionId, setEditorRuntimeStateBySessionId] =
    useState<Map<string, EditorSessionRuntimeState>>(new Map());
  ...
  const updateEditorSessionStatus = useCallback(
    (sessionId: string, status: EditorSession["status"]) => {
  ...
  const fetchEditorSessionDiff = useCallback(
    async (sessionId: string, mode: ChangesMode) => {
  ...
  const ensureEditorSessionLoaded = useCallback(
    async (sessionId: string, options?: { force?: boolean }) => {
      ...
      const requestId =
        (editorLoadRequestIdBySessionIdRef.current.get(sessionId) ?? 0) + 1;
  ...
  const applyEditorSessionViewState = useCallback(
    async (sessionId: string, viewState: EditorSessionViewState) => {
      const session = editorSessionsRef.current.get(sessionId);
      if (!session) {
        return;
      }
  ...
  const changeEditorSessionContent = useCallback(
    (sessionId: string, next: string) => {
      ...
      const current =
        editorRuntimeStateBySessionIdRef.current.get(sessionId) ??
  ...
  const openOrReuseEditorSession = useCallback(
    (input: OpenOrReuseEditorSessionInput) => {
      ...
      setEditorRuntimeStateBySessionId((previous) => {
        ...
        next.set(
          session.id,
          buildDefaultRuntimeState(
            input.preferredTab === "diff" ? "diff" : "edit"
          )
        );
        return next;
      });
  ...
  const closeEditorSession = useCallback(
    (sessionId: string, options?: { skipConfirm?: boolean }) => {
      const session = editorSessionsRef.current.get(sessionId);
      if (!session) {
        return false;
      }
      ...
      if (
        !options?.skipConfirm &&
        (session.status === "active" || isRuntimeDirty(runtimeState))
      ) {
  ...
  const closeSessionForFilePath = useCallback(
    (filePath: string, options?: { skipConfirm?: boolean }) => {
      const existing = findEditorSessionByFilePath(
        editorSessionsRef.current as unknown as Map<string, WorkspaceSession>,
        filePath
      );
  ...
  return {
    ...
    closeSessionsForProject: (projectId: number) => {
      closeMatchingSessions((session) => session.projectId === projectId, {
        skipConfirm: true,
      });
    },
    closeSessionsForDivergence: (divergenceId: number) => {
      closeMatchingSessions(
        (session) =>
          session.targetType === "divergence" && session.targetId === divergenceId,
        { skipConfirm: true }
      );
    },
    closeSessionsForWorkspace: (workspaceId: number) => {
      closeMatchingSessions(
        (session) =>
          session.targetType === "workspace" && session.targetId === workspaceId,
        { skipConfirm: true }
      );
    },
