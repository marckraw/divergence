...
import { readDir, readTextFile } from "../api/fs.api";
import { getImportPathMatchFromPrefix } from "../lib/importPathMatch.pure";
import {
  buildImportLabel,
  getDirname,
...
function getImportPathMatch(context: CompletionContext) {
...
async function readDirCached(path: string) {
  const now = Date.now();
...
async function readPackageNamesCached(rootPath: string) {
  const now = Date.now();
...
async function getRelativePathCompletions(filePath: string, typed: string): Promise<Completion[]> {
...
async function getPackageCompletions(rootPath: string, typed: string): Promise<Completion[]> {
...
function createImportPathCompletionSource(filePath: string | null, projectRootPath: string | null) {
...
function buildCompletionExtensions(filePath: string | null, projectRootPath: string | null) {
