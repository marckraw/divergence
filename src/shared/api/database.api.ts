...
} from "../lib/databaseMigrations.pure";
...
async function recoverInterruptedMigration14(
  database: Database,
  currentVersion: number,
): Promise<number> {
  const hasAutomations = await tableExists(database, "automations");
  const hasAutomationsV14 = await tableExists(database, "automations_v14");
  const action = getMigration14RecoveryAction({
    currentVersion,
    hasAutomations,
    hasAutomationsV14,
  });
  ...
  if (action === "drop_stale_v14") {
    await database.execute("DROP TABLE IF EXISTS automations_v14");
    return currentVersion;
  }

  await database.execute("PRAGMA foreign_keys = OFF");
  try {
    await database.execute("ALTER TABLE automations_v14 RENAME TO automations");
    await database.execute(
      "CREATE INDEX IF NOT EXISTS idx_automations_project_id ON automations(project_id)",
    );
    await database.execute(
    ...
    await database.execute("INSERT INTO _schema_version (version) VALUES ($1)", [14]);
  } finally {
    await database.execute("PRAGMA foreign_keys = ON");
  }
...
const MIGRATIONS: Migration[] = [
    ...
    statements: [
      ...
      },
      {
        sql: `CREATE TABLE IF NOT EXISTS automations (
          ...
          name TEXT NOT NULL,
          project_id INTEGER NOT NULL,
          agent TEXT NOT NULL CHECK(agent IN ('claude', 'codex', 'cursor', 'gemini', 'opencode')),
  ...
  {
    version: 14,
    description: "Expand automation agent enum for OpenCode",
    statements: [
      { sql: "PRAGMA foreign_keys = OFF" },
      {
        sql: `CREATE TABLE IF NOT EXISTS automations_v14 (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          name TEXT NOT NULL,
          project_id INTEGER NOT NULL,
          agent TEXT NOT NULL CHECK(agent IN ('claude', 'codex', 'cursor', 'gemini', 'opencode')),
      ...
      {
        sql: `INSERT INTO automations_v14 (
          id, name, project_id, agent, prompt, interval_hours, run_mode, source_project_id,
          target_project_id, trigger_type, trigger_config_json, enabled, keep_session_alive,
          last_run_at_ms, next_run_at_ms, created_at_ms, updated_at_ms, workspace_id
        )
      ...
      { sql: "DROP TABLE automations" },
      { sql: "ALTER TABLE automations_v14 RENAME TO automations" },
      { sql: "CREATE INDEX IF NOT EXISTS idx_automations_project_id ON automations(project_id)" },
      { sql: "CREATE INDEX IF NOT EXISTS idx_automations_run_mode ON automations(run_mode)" },
      { sql: "PRAGMA foreign_keys = ON" },
    ],
...
async function runMigrations(database: Database): Promise<void> {
  ...
  currentVersion = await recoverInterruptedMigration14(database, currentVersion);
