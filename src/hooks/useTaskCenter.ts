...
import type { BackgroundTask, BackgroundTaskKind, BackgroundTaskTarget, BackgroundTaskToast } from "../types";
import {
  getRecentTasks,
  getRunningTasks,
  normalizeUnknownError,
} from "../lib/utils/taskCenter";
...
interface RunTaskOptions<T> {
  ...
  fsHeavy: boolean;
  initialPhase?: string;
  run: (controls: TaskControls) => Promise<T>;
...
interface UseTaskCenterResult {
  ...
  runTask: <T>(options: RunTaskOptions<T>) => Promise<T>;
}
...
export function useTaskCenter(fsConcurrency: number = DEFAULT_FS_CONCURRENCY): UseTaskCenterResult {
  ...
  const runTask = useCallback(<T,>(options: RunTaskOptions<T>): Promise<T> => {
